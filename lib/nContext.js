...
function NContext() {
    let args = new Args([
        ...
        {
            precision: Args.Optional | Args.STRING,
            _check: function (v) {
                return v === "single" || v === "double";
            }
        },
        { kernelBuildOptions: Args.Optional | Args.OBJECT }
    ], arguments);

    this.options = {
        deviceHint: args.deviceHint,
        precision: args.precision || "single",
        kernelBuildOptions: args.kernelBuildOptions || {}
    };

    this.clHost = CLHost.createV11();
    this.clDevice = findDevice(this.clHost, this.options.deviceHint);
    this.clContext = new CLContext(this.clDevice);
    this.clQueue = new CLCommandQueue(this.clContext, this.clDevice);
}

Object.defineProperties(NContext.prototype, {
    realType: {
        get: function() {
            return this.options.precision === "double" ? "double" : "float";
        }
    }
});
