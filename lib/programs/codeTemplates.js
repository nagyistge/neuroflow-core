"use strict";

var fs = require("fs");
var _ = require("lodash");
var path = require("path");

var templateKey = "sdjf3ujq9gjgopigjqpog";

function loadTemplate(file) {
    global[templateKey] = global[templateKey] || {};
    var cache = global[templateKey];
    var template = cache[file];
    if (_.isString(template)) {
        return template;
    }
    template = fs.readFileSync(path.join(__dirname, file), "utf8");
    if (template.length === 0) {
        throw new Error("Template '" + file + "' is empty.");
    }
    if (template[template.length - 1] !== "\n") {
        template += "\n";
    }
    cache[file] = template;
    return template;
}

var codeTemplates = {
    localBarrier: function() {
        return "barrier(CLK_LOCAL_MEM_FENCE);\n";
    },

    globalBarrier: function(split) {
        if (split) {
            return "\n--------------------------------------------------------\nbarrier(CLK_GLOBAL_MEM_FENCE);\n--------------------------------------------------------\n";
        }
        else {
            return "barrier(CLK_GLOBAL_MEM_FENCE);\n";
        }
    },

    forEachGlobalBegin: function() {
        return loadTemplate(path.join("templates", "forEachGlobalBegin.cl"));
    },

    forEachGlobalEnd: function() {
        return loadTemplate(path.join("templates", "forEachGlobalEnd.cl"));
    },

    gradientDescent: function(optionsPropertyName) {
        optionsPropertyName = optionsPropertyName || "gradientDescent";
        return loadTemplate(path.join("templates", "gradientDescent.cl")).replace(new RegExp("options.", "g"), "options." + optionsPropertyName + ".");
    },

    vectorAdapter_InputCodeForValues: function(optionsPropertyName) {
        optionsPropertyName = optionsPropertyName || "vectorAdapter_InputCodeForValues";
        return loadTemplate(path.join("templates", "vectorAdapter_InputCodeForValues.cl")).replace(new RegExp("options.", "g"), "options." + optionsPropertyName + ".");
    },

    vectorAdapter_InputCodeForBuffers: function(optionsPropertyName) {
        optionsPropertyName = optionsPropertyName || "vectorAdapter_InputCodeForBuffers";
        return loadTemplate(path.join("templates", "vectorAdapter_InputCodeForBuffers.cl")).replace(new RegExp("options.", "g"), "options." + optionsPropertyName + ".");
    }
};

module.exports = codeTemplates;