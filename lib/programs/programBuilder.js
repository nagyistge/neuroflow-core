...
ProgramBuilder.prototype.addPart = function (part) {
    assert(part);
    assert(_.isString(part.kernelName));
...
ProgramBuilder.prototype.build = async(function*() {
    ...
    let addPartToProgram = async(function*(part) {
        ...
        let builder = new KernelBuilder({
            kernelName: part.kernelName,
            precision: self.nContext.options.precision,
            args: part.args,
            template: code,
            templateOptions: part.templateOptions
        });
        ...
        programLauncher.addPart(kernelLauncher, new NDRange(part.globalSize), new NDRange(part.localSize));
    });
